...
export default function SimulationPage() {
  ...
  const [alternativeRoutesLineData, setAlternativeRoutesLineData] = useState<
    LineData[]
  >([]);
  const [isDrivingDirectionEnabled, setIsDrivingDirectionEnabled] =
    useState(false);
  ...
  const onSimulationStart = useCallback(
    async (
              ...
              if (!currentGpsLocRef.current) {
                ...
                await new Promise<void>((resolve) => {
                  ...
                  gsap.to(animationData, {
                    ...
                    onUpdate: () => {
                      currentGpsLocRef.current = {
                        lat: animationData.lat,
                        lon: animationData.lon,
                      };
              ...
              if (drivingDirection && routeDataRef.current.length > 0) {
                ...
                if (usedRoute && usedRoute.driving_directions.length > 0) {
                  ...
                  const safeIndex = Math.min(
                    Math.max(0, directionsIndex),
                    usedRoute.driving_directions.length - 1,
                  );
                    ...
                    void (async () => {
                      try {
                        ...
                        const newAlternatives =
                          altResponse.data.alternative_routes;
                        if (newAlternatives.length > 0) {
                          ...
                          const alternativesPolyline = newAlternatives.map(
                            (route) => {
                        ...
                        console.error(
                          "Failed to fetch alternatives dynamically:",
                          e,
                        );
                      }
                    })();
                  ...
                  if (isOffTheRoute && currentEdgeID !== -1) {
                    ...
                    const otherRouteIndex = routeDataRef.current.findIndex(
                      (route, idx) =>
                  ...
                  if (isOffTheRoute && currentEdgeID !== -1) {
                    try {
                      ...
                      if (
                        processedRoutes.alternativeRoutesLineData.length > 0
                      ) {
                        setAlternativeRoutesLineData(
                          processedRoutes.alternativeRoutesLineData,
                        );
      ...
      toast.success("Simulation finished", { duration: 1000 });
    },
  ...
  return (
    <main className="flex relative w-full overflow-hidden">
            ...
            <div className="flex gap-4">
              <button
                onClick={() => {
                  ...
                  toast.success(
                    newVal
                      ? "WebSocket enabled (will apply on next request if possible)"
                      : "WebSocket disabled",
                  );
                }}
              ...
              >
                <div
                  className={`w-3.5 h-3.5 rounded-full flex items-center justify-center transition-all ${isUsingWebSocket ? "bg-blue-500" : "border border-gray-400"}`}
                >
              ...
              >
                <div
                  className={`w-3.5 h-3.5 rounded-full flex items-center justify-center transition-all ${isShowingGpsWindow ? "bg-blue-500" : "border border-gray-400"}`}
                >
              ...
              >
                <div
                  className={`w-3.5 h-3.5 rounded-full flex items-center justify-center transition-all ${isUsingWasm ? "bg-blue-500" : "border border-gray-400"}`}
                >
              ...
              <div className="bg-[#222831]/95 backdrop-blur-md rounded-2xl p-4 shadow-xl flex items-center gap-4 border border-white/10">
                ...
                <div className="flex flex-col">
                  <p className="text-xl font-black text-white leading-tight">
                    {distanceFromNextTurnPoint.toFixed(0)}{" "}
                    <span className="text-sm font-normal opacity-70">m</span>
                  </p>
      ...
      <div
        className={
          isRunning && isDrivingDirectionEnabled ? "hidden sm:block" : "block"
        }
      >
        <SimulationPanel
